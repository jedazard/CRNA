...
#==========================================================================================#
CODE.path <- "CODES"
DATA.path <- "DATASETS/METHODS/CRNA/REAL/EGF_Study"
...
#==========================================================================================#
#save.image(file=file.path(HOME.path, RESULT.path, "CRNA_REAL_EGF.RData", fsep = .Platform$file.sep))

#==========================================================================================#
# Load the workspace
#==========================================================================================#
load(file=file.path(HOME.path, RESULT.path, "CRNA_REAL_EGF.RData", fsep = .Platform$file.sep))
...
EXP <- readRDS(file.path(HOME.path, DATA.path, "GSE60880.RDS", fsep = .Platform$file.sep))
X <- Biobase::exprs(EXP)
...
e <- igraph::ecount(PPI.ig)  # e=10789 final edges
p <- igraph::vcount(PPI.ig)  # p=3282 final nodes
...
fdr: 1e-07
d/p: 0.00670322973796
fdr: 1e-06
d/p: 0.0100548446069
fdr: 1e-05
d/p: 0.0118829981718
Median degree (\nu): 3
Median path length (\lambda): 3.646
e: 10789
p: 3282
e/p: 3.28732480195
...
#==========================================================================================#
TF <- factor(c(rep(1,24), 
               rep(2,24)), ordered=FALSE, labels=c("T", "C"))
TF.lev <- levels(TF)
TF.ng <- nlevels(TF)
...
DF <- factor(c(rep(1,6), rep(2,6), 
               rep(3,6), rep(4,6),
               rep(1,6), rep(2,6), 
               rep(3,6), rep(4,6)), ordered=FALSE, labels=c("0.5h", "1h", "2h", "8h"))
DF.lev <- levels(DF)
DF.ng <- nlevels(DF)
...
DT <- factor(c(rep(1,6), rep(2,6),
               rep(3,6), rep(4,6),
               rep(5,6), rep(6,6),
               rep(7,6), rep(8,6)), ordered=FALSE, labels=as.character(outer(X=DF.lev, Y=TF.lev, FUN=function(x,y) {paste(x,y, sep=".")})))
DT.lev <- levels(DT)
DT.ng <- nlevels(DT)
...
sample.names <- c(paste0(DT.lev[1], ".", DT.def[[1]]),
                  paste0(DT.lev[2], ".", DT.def[[2]]),
                  paste0(DT.lev[3], ".", DT.def[[3]]),
                  paste0(DT.lev[4], ".", DT.def[[4]]),
                  paste0(DT.lev[5], ".", DT.def[[1]]),
                  paste0(DT.lev[6], ".", DT.def[[2]]),
                  paste0(DT.lev[7], ".", DT.def[[3]]),
                  paste0(DT.lev[8], ".", DT.def[[4]]))

#==========================================================================================#
# Drop outs
#==========================================================================================#
TF <- factor(c(rep(1,12), 
               rep(2,12)), ordered=FALSE, labels=c("T", "C"))
TF.lev <- levels(TF)
TF.ng <- nlevels(TF)
TF.def <- vector(mode="list", length=TF.ng)
TF.tab <- numeric(TF.ng)
...
DF <- factor(c(rep(2,6), rep(3,6),
               rep(2,6), rep(3,6)), ordered=FALSE, labels=c("1h", "2h"))
DF.lev <- levels(DF)
DF.ng <- nlevels(DF)
DF.def <- vector(mode="list", length=DF.ng)
DF.tab <- numeric(DF.ng)
...
DT <- factor(c(rep(2,6), rep(3,6), 
               rep(6,6), rep(7,6)), ordered=FALSE, labels=as.character(outer(X=DF.lev, Y=TF.lev, FUN=function(x,y) {paste(x,y, sep=".")})))
DT.lev <- levels(DT)
DT.ng <- nlevels(DT)
DT.def <- vector(mode="list", length=DT.ng)
DT.tab <- numeric(DT.ng)
...
# Updating final Matrix dimensions
#==========================================================================================#
n <- ncol(X)  # n=24 samples
p <- nrow(X)  # p=3282 nodes/variables
...
quant.set <- rev(1-seq(from=0.001, to=0.05, by=0.001))
...
my.file <- paste("/CV PE profile as a function of restart probability (alpha), for fixed size (q=", size.set[q], ") and DE FDR (fdr=", format(x=fdr.set[f], nsmall=2, scientific=TRUE), ") - EGF Study.ps", sep="")
...
my.file <- paste("/CV PE profile as a function of size (q), for fixed restart probability (alpha=", amin[[f]], ") and DE FDR (fdr=", format(x=fdr.set[f], nsmall=2, scientific=TRUE), ") - EGF Study.ps", sep="")
postscript(file=paste(HOME.path, RESULT.path, my.file, sep="/"), horizontal=F, width=8.5, height=6)
...
my.file <- paste("/CV PE surface as a function of size (q), for fixed restart probability (alpha=", amin[[f]], ") and DE FDR (fdr=", format(x=fdr.set[f], nsmall=2, scientific=TRUE), ") - EGF Study.ps", sep="")
...
my.file <- paste("/CV PE profile as a function of quant (1-q), for fixed restart probability (alpha=", amin[[f]], ") and DE FDR (fdr=", format(x=fdr.set[f], nsmall=2, scientific=TRUE), ") - EGF Study.ps", sep="")
postscript(file=paste(HOME.path, RESULT.path, my.file, sep="/"), horizontal=F, width=8.5, height=6)
...
write.table(CRNA.output$edges, 
            file.path(HOME.path, RESULT.path, paste("CRNA_EGF_reduced_edgelist,fdr=", format(x=fdr.set[f], nsmall=2, scientific=TRUE), ".txt", sep=""), fsep=.Platform$file.sep),
            quote=FALSE, 
            row.names=FALSE, 
            sep="\t")
write.table(CRNA.output$states, 
            file.path(HOME.path, RESULT.path, paste("CRNA_EGF_reduced_nodelist,fdr=", format(x=fdr.set[f], nsmall=2, scientific=TRUE), ".txt", sep=""), fsep=.Platform$file.sep), 
            quote=FALSE, 
            row.names=FALSE, 
            sep="\t")
...
write.table(IPA.output$edges, 
            file.path(HOME.path, RESULT.path, paste("IPA_EGF_reduced_edgelist,fdr=", format(x=fdr.set[f], nsmall=2, scientific=TRUE), ".txt", sep=""), fsep=.Platform$file.sep),
            quote=FALSE, 
            row.names=FALSE, 
            sep="\t")
write.table(IPA.output$nodes, 
            file.path(HOME.path, RESULT.path, paste("IPA_EGF_reduced_nodelist,fdr=", format(x=fdr.set[f], nsmall=2, scientific=TRUE), ".txt", sep=""), fsep=.Platform$file.sep), 
            quote=FALSE, 
            row.names=FALSE, 
            sep="\t")
...
write.table(CAUSALR.output$edges, 
            file.path(HOME.path, RESULT.path, paste("CAUSALR_EGF_reduced_edgelist,fdr=", format(x=fdr.set[f], nsmall=2, scientific=TRUE), ".txt", sep=""), fsep=.Platform$file.sep),
            quote=FALSE, 
            row.names=FALSE, 
            sep="\t")
write.table(CAUSALR.output$nodes, 
            file.path(HOME.path, RESULT.path, paste("CAUSALR_EGF_reduced_nodelist,fdr=", format(x=fdr.set[f], nsmall=2, scientific=TRUE), ".txt", sep=""), fsep=.Platform$file.sep), 
            quote=FALSE, 
            row.names=FALSE, 
            sep="\t")
...
my.file <- file.path(HOME.path, NATM.path, "Figure16.ps", fsep=.Platform$file.sep)
...
my.file <- file.path(HOME.path, NATM.path, "Figure17.ps", fsep=.Platform$file.sep)
...
write.table(CRNA.output$edges, 
            file.path(HOME.path, NATM.path, paste("Figure18_edgelist_CRNA_EGF,fdr=", format(x=fdr.set[f], nsmall=2, scientific=TRUE), ".txt", sep=""), fsep=.Platform$file.sep),
            quote=FALSE, 
            row.names=FALSE, 
            sep="\t")
write.table(CRNA.output$states, 
            file.path(HOME.path, NATM.path, paste("Figure18_nodelist_CRNA_EGF,fdr=", format(x=fdr.set[f], nsmall=2, scientific=TRUE), ".txt", sep=""), fsep=.Platform$file.sep), 
            quote=FALSE, 
            row.names=FALSE, 
            sep="\t")
...
write.table(IPA.output$edges, 
            file.path(HOME.path, NATM.path, paste("Figure20_edgelist_IPA_EGF,fdr=", format(x=fdr.set[f], nsmall=2, scientific=TRUE), ".txt", sep=""), fsep=.Platform$file.sep),
            quote=FALSE, 
            row.names=FALSE, 
            sep="\t")
write.table(IPA.output$nodes, 
            file.path(HOME.path, NATM.path, paste("Figure20_nodelist_IPA_EGF,fdr=", format(x=fdr.set[f], nsmall=2, scientific=TRUE), ".txt", sep=""), fsep=.Platform$file.sep), 
            quote=FALSE, 
            row.names=FALSE, 
            sep="\t")
...
write.table(CAUSALR.output$edges, 
            file.path(HOME.path, NATM.path, paste("Figure22_edgelist_CAUSALR_EGF,fdr=", format(x=fdr.set[f], nsmall=2, scientific=TRUE), ".txt", sep=""), fsep=.Platform$file.sep),
            quote=FALSE, 
            row.names=FALSE, 
            sep="\t")
write.table(CAUSALR.output$nodes, 
            file.path(HOME.path, NATM.path, paste("Figure22_nodelist_CAUSALR_EGF,fdr=", format(x=fdr.set[f], nsmall=2, scientific=TRUE), ".txt", sep=""), fsep=.Platform$file.sep), 
            quote=FALSE, 
            row.names=FALSE, 
            sep="\t")
