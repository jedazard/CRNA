...
r1_RWR0.75_DFS_graph <- RWR_dfs("EGFR",significant_genes_graph,miarray)
...
GSE60880_EGFR_graph <- graph_from_data_frame(unique(FC1.5_undirected_RWR0.75_JTREE_edgeList),directed = T)
...
#########################################################################################
RWR_GSE60880 <- read.table(paste0("data_for_markdown/Jan25_GSE60880_EGFR_GSE60880.txt"),comment.char = ">")
RWR_permuted <- read.table(paste0("data_for_markdown/Jan25_GSE60880_EGFR_permuted.txt"),comment.char = ">")
...
Tx8_majority_vote <- as.data.frame(sign(phenotypes[,8]-phenotypes[,4]) ,row.names = rownames(phenotypes))
colnames(Tx8_majority_vote) <- c("node_sign")
Tx8_majority_vote_graph <- induced.subgraph(GSE60880_EGFR_graph,as.character(rownames(Tx8_majority_vote)[which(Tx8_majority_vote$node_sign != 0)]))
Tx8_majority_vote_graph <- induced.subgraph(Tx8_majority_vote_graph,names(which(degree(Tx8_majority_vote_graph,V(Tx8_majority_vote_graph)) >0)))

plot(Tx8_majority_vote_graph, edge.arrow.size=.2, edge.curved=0,
     
     vertex.color="orange", vertex.frame.color="#555555",
     
     vertex.label=V(Tx8_majority_vote_graph)$name, vertex.label.color="black",
     
     vertex.label.cex=.5,layout = layout_nicely(Tx8_majority_vote_graph),vertex.size = 9)

Tx8_majority_vote_edgeList <- igraph::as_data_frame(Tx8_majority_vote_graph)

Tx8_majority_Vote_nodeList <- Tx8_majority_vote
Tx8_majority_Vote_nodeList <- cbind(rownames(Tx8_majority_vote),Tx8_majority_vote)
colnames(Tx8_majority_Vote_nodeList) <- c("id","node_sign")
