...
class TFactor {
    ...
    //@{
        ...
        TFactor( const VarSet& vars, const TProb<T> &p ) : _vs(vars), _p(p) {
            DAI_ASSERT( _vs.nrStates() == (BigInt)(static_cast<unsigned long>(_p.size())) );
        }
        ...
        TFactor( const std::vector<Var> &vars, const std::vector<T> &p ) : _vs(vars.begin(), vars.end(), vars.size()), _p(p.size()) {
            ...
            for( size_t i = 0; i < vars.size(); i++ )
                nrStates *= (BigInt)(static_cast<unsigned long>(vars[i].states()));
            DAI_ASSERT( nrStates == (static_cast<unsigned long>(p.size())) );
