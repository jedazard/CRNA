...
void readInteractionMap(istream& is,
  ...
  while(getline(is, line)) {
    vector< string > vals;
    vals=dai::tokenizeString(line, true);
    if (vals.size() != 4) {
...
PathwayTab::PathwayTab(DataFrame pathwayDf1, DataFrame pathwayDf2,istream& imap_stream,istream& dogma_stream,const PropertySet& props): _nodemap(),
    _nodevector(),
    _parents(),
    _entities(),
    _dogma(dogma_stream),
  ...
  //add the entities
  CharacterVector ch1 = pathwayDf1["V1"];
  CharacterVector ch2 = pathwayDf1["V2"];
  for(int i = 0; i < ch1.length();i++){
    vector< string > temp;
    temp.push_back(as<std::string>(ch1[i]));
    temp.push_back(as<std::string>(ch2[i]));
    entity_lines.push_back(temp);
  }
  //add the interactions
  CharacterVector ch3 = pathwayDf2["V1"];
  CharacterVector ch4 = pathwayDf2["V2"];
  CharacterVector ch5 = pathwayDf2["V3"];
  for(int i = 0; i < ch3.length();i++){
    vector< string > temp;
    temp.push_back(as<std::string>(ch3[i]));
    temp.push_back(as<std::string>(ch4[i]));
    temp.push_back(as<std::string>(ch5[i]));
    interaction_lines.push_back(temp);
  }
  //CHANGE THIS SUCH THAT every row in nodesDf (entity) and edgesDf (interaction) is passed here (pushed back accordingly) as a vector of strings, THEN WE ARE DONE HERE. change function parameters in both .h and .cpp carefully. 
  //wherever needed. dont forget rcpp export. then, that's it for pathwaytab??? get nodesDf and edgesDf as inputs to JTInf function (formerly known as main :) ) NEXT IS EVIDENCE
