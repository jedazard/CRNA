...
vector<SmallSet<size_t> > GLC::calcRegions() {
    ...
    // non-partitioning: overlapping (variable+markov blanket)s
        ...
        while( remaining.size() > 0 ) {
            // not good in case of an r interface (you would want to use r's rng then) but this is not the case for here and now
            std::time_t now = std::time(0);
            boost::random::mt19937 gen{static_cast<std::uint32_t>(now)};
            size_t cv = remaining.elements()[ gen() % remaining.size() ];
