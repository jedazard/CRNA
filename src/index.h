    ...
    public:
        ...
        State( const VarSet &vs, BigInt linearState=0 ) : state(linearState), states() {
            if( linearState == 0 )
                ...
                for( VarSet::const_iterator v = vs.begin(); v != vs.end(); v++ ) {
                    states[*v] = BigInt_size_t( linearState % (BigInt)(static_cast<unsigned long>(v->states())) );
                    linearState /= (BigInt)(static_cast<unsigned long>(v->states()));
                }
        ...
        BigInt operator() ( const VarSet &vs ) const {
            ...
            for( VarSet::const_iterator v = vs.begin(); v != vs.end(); v++ ) {
                ...
                if( entry != states.end() )
                    ...
                    vs_state += (BigInt)(static_cast<unsigned long>(entry->second)) * prod;
                
                prod *= (BigInt)(static_cast<unsigned long>(v->states()));
            }
            return vs_state;
        }
