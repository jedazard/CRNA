...
IPA_dfs <- function(r1,omniNet,miarray){
  passedNodes <- c(r1)
  passedEdges <- c()
  regulatedDEG <- 0
  ### Find r2
  possible_r2 <- unique(neighbors(omniNet,r1, "out")$name)
  possible_r2 <- possible_r2[!possible_r2 %in% passedNodes]      ### That's r2
  ...
  if (length(possible_r2)>0) {  ### r1 has r2
    for (i in 1:length(possible_r2)) {
      ...
      if (r2 %in% rownames(miarray)) {
        # passedEdges <- c(passedEdges, get.edge.ids(omniNet,c(r1, r2)))
        regulatedDEG <- regulatedDEG + 1
      }else{
        possible_r3 <- unique(neighbors(omniNet,r2,"out")$name)
        possible_r3 <- possible_r3[!possible_r3 %in% passedNodes]      ### That's r3
        ###if r2 has childs(r3)
        if (length(possible_r3)>0) {  ### r2 has r3
          for (i in 1:length(possible_r3)) {
            ...
            if (r3 %in% rownames(miarray)) {
              # passedEdges <- c(passedEdges, get.edge.ids(omniNet,c(r1, r2)),
              # get.edge.ids(omniNet,c(r2, r3)))             
              passedEdges <- c(passedEdges, get.edge.ids(omniNet,c(r1, r2)))
              regulatedDEG <- regulatedDEG + 1
            }else{
              possible_r4 <- unique(neighbors(omniNet,r3,"out")$name)
              possible_r4 <- possible_r4[!possible_r4 %in% passedNodes]      ### That's r4
              ###if r3 has childs(r4)
              if (length(possible_r4)>0) {  ### r3 has r4
...
omnipath <- read.csv("data_for_markdown/omnipath.final.csv",row.names = 1)
DEGList <- read.csv("data_for_markdown/GSE11352_DEGList.csv",row.names = 1)
GSE11352_exprs_for_PARADIGM <- read.csv("data_for_markdown/GSE11352_exprs_for_PARADIGM.csv",row.names = 1)[,-1]
...
ggplot(data = IPAresult_FC2, aes(x =pValue)) +
  geom_histogram(color = "white", bins = 80,fill = "royalblue") +
  labs(title = "Distribution of IPA overlapping p-value",
       x = "Overlapping p-value",y = "Frequency")
...
ggplot(data = IPAresult_FC2P0.05, aes(x =Zscore)) +
  geom_histogram(color = "white", bins = 40,fill = "royalblue") +
...
print(paste("Among significant genes,",nrow(IPAresult_FC2P0.05_Z1),
            "genes' |Z| >= 1,",nrow(IPAresult_FC2P0.05_Z0.5),
            "genes' |Z| >= 0.5,",nrow(IPAresult_FC2P0.05_Z0.25),
            "genes' |Z| >= 0.25"))

# get FC2 P0.05 Z0.5 plot 
IPAresult_FC2P0.05_Z0.5_graph <- induced.subgraph(induced.subgraph(omniNet, IPAresult_FC2P0.05_Z1$SYMBOL),
                                                  names(which(degree(IPAresult_FC2P0.05_Z0.5_graph,
                                                                     V(IPAresult_FC2P0.05_Z0.5_graph)) >0)))
IPAresult_FC2P0.05_Z0.5_edgeList <- igraph::as_data_frame(IPAresult_FC2P0.05_Z0.5_graph)

print(paste("When |FC| >= 2, p-value >= 0.05, Z >= 0.5,  There are",nrow(IPAresult_FC2P0.05_Z0.5_edgeList),
            "edges,",length(unique(c(IPAresult_FC2P0.05_Z0.5_edgeList$from,IPAresult_FC2P0.05_Z0.5_edgeList$to))),
